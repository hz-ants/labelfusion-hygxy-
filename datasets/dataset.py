...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num, add_noise, root, noise_trans, refine):
        self.objlist = [1, 2, 3] #1. flansch 2.schaltgabel 3.stift
        self.mode = mode
        ...
        for item in self.objlist:
            ...
            #print(self.list_obj)
            meta_file = open('{0}/data/{1}/posedata.yml'.format(self.root, '%02d' % item), 'r')
...
#       [False, False, False, ...,  True,  True,  True]])
        ...
        target = np.dot(model_points, target_r.T)      #roatate model_points with target_r   #target.shape = (500, 3)


        if self.add_noise:
            target = np.add(target, target_t / 1000.0 + add_t)  # translate and add noise on model_points
        ...
        #cloud -> calculated point cloud with camera intrinisic parameters on randomly chosen pixel points within cropped area
        #choose -> indices of 500 randomly chosen pixel points 
        #img_masked ->cropped color image with/without argumentation
        #target ->500 randomly chosen model points under rotaiton and translation of grounf truth pose 
        #model_points -> 500 randomly chosen model points before rotation and translation 
        #obj ->index to tell which object we are dealing with
